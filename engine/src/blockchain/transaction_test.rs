...
mod tests {
    use crate::{
        blockchain::{blockchain::BlockChain, transaction::Transaction, utxo::UnspentOutput},
    ...
    fn balance_transaction_happy_path() {
        ...
        assert_eq!(
            transaction.inputs[0].address, *pub_1,
        ...
        assert_eq!(
            transaction.outputs[0].address, *pub_2,
            "To address is wrong"
        );
        ...
        let block = blockchain
            .commit_transaction(&signed_transaction, validator_private_key)
            .unwrap();
        assert_eq!(
            blockchain.blocks.len(),
            1,
        ...
        assert_eq!(
            ...
            1,
            "Number of signatures is wrong"
        );
        assert_eq!(
            block.validator_signatures()[0].validator_public_key,
        ...
        assert_eq!(
            transaction.outputs()[1].address,
            *pub_1,
            "Change address is wrong"
        );
        ...
        assert_eq!(
            transaction.inputs()[0].amount,
            5,
            "First input amount is wrong"
        );
        assert_eq!(
            transaction.inputs()[1].address,
            *pub_1,
            "From address is wrong"
        );
        assert_eq!(
