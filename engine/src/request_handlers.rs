...
use protocol::{
    request::{CommandResponse, CommandRequest, Validator, ValidatorSignature, self, Response}, request::Request,
};
    ...
    match &request.command {
        ...
        CommandRequest::RequestTransactionValidation { blockchain_previous_tip, blockchain_new_tip, transaction_cbor, validator_signature } => {
            ...
            if *blockchain_previous_tip != blockchain_hash {
                ...
                println!("{}", msg);
                return ok_no_requests(Response::Error { msg });
            }
            ...
            ok_no_requests(Response::Success {
                request_id: request.request_id.to_owned(),
        ...
        CommandRequest::CreateRecord { data } => panic!("Not ready yet"),
...
fn ok_no_requests(response: Response) -> Result<(Response, Vec<(ValidatorPublicKeyAndAddress, Request)>)> {
    Ok((response, Vec::new()))
}

fn ok_with_requests(response: Response, requests: Vec<(ValidatorPublicKeyAndAddress, Request)>) -> Result<(Response, Vec<(ValidatorPublicKeyAndAddress, Request)>)> {
